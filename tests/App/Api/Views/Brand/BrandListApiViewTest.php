...
/**
 * Class BrandListApiViewTest
 * @package Tests\App\Api\Views\Brand
 */
class BrandListApiViewTest extends DbUnitTestCase
{
    ...
    public function testRouteResponseSuccessfully()
    {
        ...
        $request = TestUtils::makeServerRequestMock('GET', '/api/brand/list');
        ...
        $this->assertEquals(20, \count($arrayBody['data']));
        ...
        $this->assertEquals('GARNIER', $item['label']);

        $itemLast = $arrayBody['data'][19];
        $this->assertEquals(20, $itemLast['value']);
        $this->assertEquals('SEPHORA', $itemLast['label']);
    }
    ...
    {
        $xmlFile = BASE_DIR . '/tests/dbunit-resources/api-brand-list-route.xml';
